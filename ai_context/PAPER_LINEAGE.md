# PAPER_LINEAGE.md — Version History for Paper 10

This file tracks the version history of Paper 10 (The Calculus of Finite Continuability) and explains the relationship between the version in this release, the version on Zenodo, and any intermediate versions.
...
- **Concept DOI:** 10.5281/zenodo.20041680
...
- **v1.21** (current): see the main paper's changelog section for details.
...
**vv1.21 supersedes all earlier versions** for current reference. If citing for strict reproducibility of work published before this version, cite the version DOI from the specific Zenodo deposit the earlier work used.
...
All `\coderef{check_X}{module.py}` anchors in this paper are valid against codebase v24.3.427. If you are reading this release against a newer canonical codebase, a handful of checks may have been renamed, merged, or relocated; consult the bank changelog in `apf/__init__.py` of the newer codebase version.

The full codebase is at:
- `__APF Library/Codebase/APF_Codebase_v24.3.427/` (this release built against this version)
...
*Generated by `create-repo`. The `- **v1.21** (current): see the main paper's changelog section for details.
