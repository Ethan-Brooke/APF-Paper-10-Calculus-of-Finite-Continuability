...
Open the interactive DAG at `https://ethan-brooke.github.io/APF-Paper-10-Calculus-of-Finite-Continuability/` (requires GitHub Pages enabled on this repo's `/docs` folder). Hover any node for its claim, epistemic tag, and shortest derivation chain. Click **Run Checks** to watch all theorems verify in topological order.
...
Expected output: one line per check, then a summary `Paper 10 (The Calculus of Finite Continuability): N passed, 0 failed, N total`. Elapsed time is typically under a second.
